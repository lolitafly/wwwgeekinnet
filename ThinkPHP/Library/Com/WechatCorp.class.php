...
class WechatCorp {
     ...
     * @var string
     */
    private $qyapiURL    = 'https://qyapi.weixin.qq.com/cgi-bin';
    ...
    public function getAccessToken(){
        ...
        if($access_token){//若存在缓存，且为调用基础access_token
        	$this->accessToken=$access_token;
        	return $access_token;
        	exit(1);
        }
        
        $url = "{$this->qyapiURL}/gettoken";
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
            	...
            	S('wx_access_token',$token['access_token'],7200);
    ...
    public function getUserInfo($access_token, $code,$agentid){
        $query = array(
        ...
        $info = self::http("{$this->qyapiURL}/user/getuserinfo", $query);
