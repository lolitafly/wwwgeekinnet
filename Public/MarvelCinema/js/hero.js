...
function gameReset(){
	...
	// 将要输出的内容
	var html = "<table  frame=\"void\" border=\"1\" cellspacing=\"0\">\n";
	// 行与列+2是因为上下左右的第一行都应为空
	var n = 0;
	for(var row = 1; row < colNum-1; row++)
	{
	    html += "<tr>\n";
	    for(var col = 1; col < rowNum-1; col++)
	    {
	        ...
	        if(matrix[row][col] > 0)
	        {
	            ...
	            html += "<img src=\"/Public/MarvelCinema/images/"+ matrix[row][col] +".png\" row="+row+" col="+col+" class=\"icon\">\n";
	            html +="<img src=\"/Public/MarvelCinema/images/wrap.png\" class=\"wrap\">\n";
	        }
	        html += "</td>\n";
	    }
	}
        ...
        else if(o != tempO)
        {
            ...
            if(matrix[pathInfo[0].x][pathInfo[0].y] == matrix[pathInfo[1].x][pathInfo[1].y])
            {
                ...
                if(feasiblePath(pathInfo[0], pathInfo[1]))
                {
                    ...
                    // 删除图片
                    $(tempO).attr("src","/Public/MarvelCinema/images/tm.png");
                    $(o).attr("src","/Public/MarvelCinema/images/tm.png");
                    //加分
                    score=score+20;
                    $("#score").text(score);
